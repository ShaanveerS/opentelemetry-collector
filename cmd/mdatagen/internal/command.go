...
const (
	...
	configStart = "<!-- configuration autogenerated section -->"
	configEnd   = "<!-- end configuration autogenerated section -->"
)
...
func run(ymlPath string) error {
	...
	resolvedConfig, err := generateConfigFiles(md, ymlDir, importRootPath)
	if err != nil {
		return fmt.Errorf("failed to generate config files: %w", err)
	}
	if resolvedConfig != nil {
		if _, err = os.Stat(filepath.Join(ymlDir, "README.md")); err == nil {
			configSection, sectionErr := renderConfigSection(resolvedConfig)
			if sectionErr != nil {
				return fmt.Errorf("failed to generate config documentation: %w", sectionErr)
			}

			err = inlineReplaceBytes(filepath.Join(ymlDir, "README.md"), configStart, configEnd, configSection)
			if err != nil {
				return err
			}
		}
...
func inlineReplace(tmplFile, outputFile string, md Metadata, start, end, goPackage, importRootPath string) error {
	...
	return inlineReplaceBytes(outputFile, start, end, buf)
}
...
func generateConfigFiles(md Metadata, mdDir, _ string) (*cfggen.ConfigMetadata, error) {
	if md.Config == nil {
		return nil, nil
	}
	...
	if err != nil {
		return nil, fmt.Errorf("failed to resolve config schema: %w", err)
	}

	err = cfggen.WriteJSONSchema(mdDir, resolvedSchema)
	if err != nil {
		return nil, fmt.Errorf("failed to write config schema: %w", err)
	}

	if err := generateConfigGoStruct(md, mdDir); err != nil {
		return nil, fmt.Errorf("failed to generate config Go struct: %w", err)
	}

	return resolvedSchema, nil
}

func inlineReplaceBytes(outputFile, start, end string, replacement []byte) error {
	readmeContents, err := os.ReadFile(filepath.Clean(outputFile))
	if err != nil {
		return err
	}

	re := regexp.MustCompile(fmt.Sprintf("%s[\\s\\S]*?%s", regexp.QuoteMeta(start), regexp.QuoteMeta(end)))
	if !re.Match(readmeContents) {
		return nil
	}

	updated := re.ReplaceAllLiteral(readmeContents, replacement)
	if err := os.WriteFile(outputFile, updated, 0o600); err != nil {
		return fmt.Errorf("failed writing %q: %w", outputFile, err)
	}
...
func renderConfigSection(md *cfggen.ConfigMetadata) ([]byte, error) {
	rendered, err := cfggen.GenerateMarkdownDocumentation(md)
	if err != nil {
		return nil, err
	}

	var buf bytes.Buffer
	buf.WriteString(configStart)
	buf.WriteString("\n")
	buf.Write(rendered)
	if len(rendered) == 0 || rendered[len(rendered)-1] != '\n' {
		buf.WriteString("\n")
	}
	buf.WriteString(configEnd)
