...
func TestRunContents(t *testing.T) {
	tests := []struct {
		...
		wantConfigReadmeGenerated       bool
		wantConfigReadmeContains        []string
		wantErr                         bool
		wantOrderErr                    bool
		wantRunErr                      bool
		...
		},
		{
			...
			wantConfigReadmeGenerated:  true,
			wantConfigReadmeContains: []string{
				"## Configuration",
				"`endpoint`",
				"`timeout`",
...
<!-- configuration autogenerated section -->
bar
<!-- end configuration autogenerated section -->`), 0o600))
			...
			if tt.wantConfigReadmeGenerated {
				require.NotContains(t, string(contents), "bar")
				for _, fragment := range tt.wantConfigReadmeContains {
					require.Contains(t, string(contents), fragment)
				}
			} else {
				require.Contains(t, string(contents), "bar")
			}
...
func TestGenerateConfigFiles(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			resolved, err := generateConfigFiles(tt.md, tmpdir, "testmodule")
			if tt.wantErr {
				require.Error(t, err)
				return
			}
...
func TestGenerateConfigFiles_GoStructError(t *testing.T) {
	...
	_, err := generateConfigFiles(md, t.TempDir(), "testmodule")
	require.Error(t, err)
...
func TestGenerateConfigFiles_WriteError(t *testing.T) {
	...
	_, err := generateConfigFiles(md, "/nonexistent/path/that/does/not/exist", "testmodule")
	require.Error(t, err)
...
func TestInlineReplaceBytes_UsesLiteralReplacement(t *testing.T) {
	outputFile := filepath.Join(t.TempDir(), "README.md")
	require.NoError(t, os.WriteFile(outputFile, []byte(`
before
<!-- configuration autogenerated section -->
old
<!-- end configuration autogenerated section -->
after
`), 0o600))

	replacement := []byte(`<!-- configuration autogenerated section -->
Use $ref and $defs literally.
<!-- end configuration autogenerated section -->`)
	require.NoError(t, inlineReplaceBytes(outputFile, configStart, configEnd, replacement))

	contents, err := os.ReadFile(outputFile)
	require.NoError(t, err)
	require.Contains(t, string(contents), "Use $ref and $defs literally.")
}
