...
It also generates Go config structs and human-readable documentation for configuration options.
...
To embed the generated configuration reference in a component README, add an autogenerated section marker:
...
When `mdatagen` runs, it replaces that block with a generated configuration section derived from the resolved schema.
